...
ROT_ETH_ADDY = util.to_checksumaddr(rot_contract_formatted_uni)
maggot_contract = '0x163c754eF4D9C03Fc7Fa9cf6Dd43bFc760E6Ce89'
MAGGOT_ETH_ADDY = util.to_checksumaddr(maggot_contract.lower())

USDC_ETH_ADDY_RAW = "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48"
USDC_ETH_ADDY = util.to_checksumaddr(USDC_ETH_ADDY_RAW)

WETH_ETH_ADDY_RAW = "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
...
def _get_1rot_price():
    price_1rot_viper = web3_calls.get_token_token_input_price(ROT_HARMONY_ADDY, 1 * 10 ** 18, VIPER_TOKEN_ADDY, viperswap_contract)
...
def _get_rot_price():
    price_rot_eth = web3_calls.get_token_token_input_price(ROT_ETH_ADDY, 1*10**18, WETH_ETH_ADDY, uniswap_contract)
    price_rot_usdc = web3_calls.get_token_token_input_price(WETH_ETH_ADDY, price_rot_eth, USDC_ETH_ADDY, uniswap_contract)
    return price_rot_eth, price_rot_usdc
...
def _get_maggot_price():
    ...
    price_maggot_usdc = price_maggot_rot * price_rot_usdc
    return price_maggot_rot, price_maggot_usdc


def get_arb(update: Update, context: CallbackContext):
    chat_id = update.message.chat_id
        ...
        if new_time - last_time_checked_price_supply > 60:
            ...
            if simple_query:
                ...
                caption = "Supply of the last " + str(time_start) + str(
                    time_type) + ".\nCurrent supply: \n<b>ROT:</b> <pre>" + current_rot_str + "</pre> \n<b>MAGGOT:</b> <pre>" + current_maggot_str + "</pre>"
            caption += "\nTotal deflation = " + str(deflation * 100)[:5] + "%"
