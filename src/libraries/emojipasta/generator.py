...
from https://github.com/Kevinpgalligan/EmojipastaBot
...
import libraries.emojipasta.util.emoji
import libraries.emojipasta.util.files
import libraries.emojipasta.util.text
...
class EmojipastaGenerator:
    ...
    def generate_emojipasta(self, text):
        blocks = libraries.emojipasta.util.text.split_into_blocks(text)
        new_blocks = []
    ...
    def _generate_emojis_from(self, block):
        trimmed_block = libraries.emojipasta.util.text.trim_nonalphabetical_characters(block)
...
def _get_emoji_mappings():
    ...
    if _EMOJI_MAPPINGS is None:
        with io.open(libraries.emojipasta.util.files.PATH_TO_MAPPINGS_FILE, "r", encoding="utf-8") as mappings_file:
